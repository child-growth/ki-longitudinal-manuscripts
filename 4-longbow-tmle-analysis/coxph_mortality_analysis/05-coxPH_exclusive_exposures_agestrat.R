...
#load data
d <- readRDS("/data/KI/UCB-SuperLearner/Manuscript analysis data/mortality_age_no_overlap_agestrat.rds")
...
d$agecat_strat<-d$agecat
res_temp <- run_cox_meta(df=d, X_vector="swast", Y="dead", Wvars=Wvars, V="agecat_strat", agecat=NULL, no_exp_overlap=T)


#All ages < 730 days
set.seed(12345)
res_mod <- res_sev <- NULL
res_mod <- run_cox_meta(df=d, X_vector=X_vector, Y="dead", Wvars=Wvars, V="agecat_strat", agecat=NULL, no_exp_overlap=T)
res_sev <- run_cox_meta(df=d, X_vector=X_vector_sev, Y="dead", Wvars=Wvars, V="agecat_strat", agecat=NULL, no_exp_overlap=T)
    
res <- bind_rows(res_mod, res_sev)
 
res
table(res$agecat_strat)


saveRDS(res, file=paste0(BV_dir,"/results/cox_results_no_overlap_agestrat.RDS"))
