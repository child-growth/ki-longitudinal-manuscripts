...
writeLines(toJSON(enumerated_analyses),"muac_mortality_analyses.json")
...
mort_batch_inputs <- "muac_mortality_analyses.json"
...
get_batch_results(mort_batch_id, results_folder="muac_results")
length(dir("results"))

# load and concatenate the rdata from the jobs
results <- load_batch_results("results.rdata", results_folder = "muac_results")
obs_counts <- load_batch_results("obs_counts.rdata", results_folder = "muac_results")

# save concatenated results
filename1 <- paste(paste('muac_mortality',Sys.Date( ),sep='_'),'RDS',sep='.')
filename2 <- paste(paste('muac_mortality_obs_counts',Sys.Date( ),sep='_'),'RDS',sep='.')
