...
get_batch_results(batch_id_cont, results_folder="seasonality_birth_results")
length(dir("seasonality_birth_results"))

# load and concatenate the rdata from the jobs
results <- load_batch_results("results.rdata", results_folder = "seasonality_birth_results")
obs_counts <- load_batch_results("obs_counts.rdata", results_folder = "seasonality_birth_results")

# save concatenated results
filename1 <- paste(paste('seasonality_birth_results',Sys.Date( ),sep='_'),'rdata',sep='.')
filename2 <- paste(paste('seasonality_birth_results_obs_counts',Sys.Date( ),sep='_'),'rdata',sep='.')
