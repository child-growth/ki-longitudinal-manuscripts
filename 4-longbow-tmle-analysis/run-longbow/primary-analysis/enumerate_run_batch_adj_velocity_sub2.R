...
table(analyses$Y)
analyses <- analyses %>% filter(Y=="y_rate_len" | Y=="y_rate_wtkg")
table(analyses$A)
analyses <- analyses %>% filter(A %in% c("birthlen","birthwt","mwtkg","mhtcm","mbmi","fhtcm"))
...
analyses <- analyses %>% filter(A %in% c("mwtkg"))
enumerated_analyses <- lapply(seq_len(nrow(analyses)), specify_longbow)
run_ki_tmle(enumerated_analyses, results_folder="results_vel_mwtkg", overwrite = F)
