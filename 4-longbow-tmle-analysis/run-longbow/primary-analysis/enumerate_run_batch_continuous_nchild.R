...
writeLines(toJSON(enumerated_analyses),"nchild_cont_analyses.json")
...
batch_inputs <- "nchild_cont_analyses.json"
...
get_batch_results(batch_id_cont, results_folder="results_nchild_cont")
length(dir("results_nchild_cont"))

# load and concatenate the rdata from the jobs
results <- load_batch_results("results.rdata", results_folder = "results_nchild_cont")
obs_counts <- load_batch_results("obs_counts.rdata", results_folder = "results_nchild_cont")

# save concatenated results
filename1 <- paste(paste('results_nchild_cont',Sys.Date( ),sep='_'),'rdata',sep='.')
filename2 <- paste(paste('results_nchild_cont_obs_counts',Sys.Date( ),sep='_'),'rdata',sep='.')
