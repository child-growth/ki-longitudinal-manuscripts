...
writeLines(toJSON(enumerated_analyses),"child_bin_analyses.json")
...
bin_batch_inputs <- "child_bin_analyses.json"
...
get_batch_results(bin_batch_id, results_folder="results_nchild_bin")
length(dir("results_nchild_bin"))

# load and concatenate the rdata from the jobs
results <- load_batch_results("results.rdata", results_folder = "results_nchild_bin")
obs_counts <- load_batch_results("obs_counts.rdata", results_folder = "results_nchild_bin")

# save concatenated results
filename1 <- paste(paste('results_nchild_bin',Sys.Date( ),sep='_'),'rdata',sep='.')
filename2 <- paste(paste('results_nchild_bin_obs_counts',Sys.Date( ),sep='_'),'rdata',sep='.')
