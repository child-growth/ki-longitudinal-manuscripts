...
plotdf_pres <- plotdf %>% filter(outcome_variable=="WLZ") %>% arrange(outcome_variable, RFgroup, title, -PAR) 
rflevels = unique(plotdf_pres$RFlabel_ref)
plotdf_pres$RFlabel_ref=factor(plotdf_pres$RFlabel_ref, levels=rflevels)

pPAR_wlz_pres <- ggplot(plotdf_pres , aes(x=RFlabel_ref, group=RFgroup, color=RFgroup)) + 
  geom_point(aes(y=-PAR),  size = 3) +
  geom_linerange(aes(ymin=-CI1, ymax=-CI2), size=1.25) +
  geom_text(aes(label=est_lab), y=-0.16, color="grey20", size=3) +
  geom_text(aes(label=est_lab_title), y=-0.16, color="black", size=3.5, fontface = "bold") +
  #coord_flip(ylim=c(-0.185,0.3)) +
  coord_flip(ylim=c(-0.225,0.2)) +
  labs(x = NULL,
       y = "") +
  geom_hline(yintercept = 0) +
  scale_y_continuous(breaks = c(-0.3,-0.2,-0.1,0,0.1,0.2,0.3,0.4), labels=c("","","","0","0.1","0.2","0.3","0.4")) +
  ...
  theme(strip.background = element_blank(),
        strip.placement = "top",
        strip.text = element_text(hjust = 0, size=12,  face="bold", color="black"),
        # axis.ticks.y = element_blank(),
        # axis.text.y = element_blank(),
        axis.text.x = element_text(size=8),
        plot.title = element_text(hjust = 0, size=9),
  ...
  ggforce::facet_col(facets = vars(RFgroup), 
                     ...
                     space = "free") 
pPAR_wlz_pres
ggsave(pPAR_wlz_pres, file=paste0(BV_dir, "/figures/risk-factor/fig-PAR-whz-pres.png"), height=10, width=8)


ggsave(pPAR_wlz, file=paste0(BV_dir, "/figures/risk-factor/fig-PAR-haz.png"), height=10, width=8)
ggsave(pPAR_laz, file=paste0(BV_dir, "/figures/risk-factor/fig-PAR-whz.png"), height=10, width=8)
