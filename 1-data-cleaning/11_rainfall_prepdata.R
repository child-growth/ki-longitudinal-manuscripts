...
monthy_ppt_values_10km <- readRDS( "~/data/KI/UCB-SuperLearner/Manuscript analysis data/monthy_ppt_values.RDS")
head(monthy_ppt_values_10km)
 ...
 df <- monthy_ppt_values_10km # %>% 
    ...
    #    )
    # ) #%>% 
    #pivot_wider(., names_from = month, values_from = avg_ppt)
# colnames(df) <- gsub("[,1]","", colnames(df) )
# head(df)                   
#  
 
 saveRDS(d, file = here("data/full_rain_data_old.rds"))
 saveRDS(df, file = here("data/full_rain_data.rds"))
