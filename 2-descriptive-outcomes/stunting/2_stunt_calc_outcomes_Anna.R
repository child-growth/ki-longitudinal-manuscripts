...
calc_outcomes = function(data, calc_method, output_file_suffix){
  ...
  # Mean HAZ
  ######################################################################
  calc_mean_haz = function(data){
    haz.data <- summary.haz.age.sex(data, method = calc_method)
    haz.region <-  data  %>% group_by(region) %>% do(summary.haz.age.sex(., method = calc_method)$haz.res)
    haz.cohort <-
      haz.data$haz.cohort %>% 
      subset(., select = c(cohort, region, agecat, sex, nmeas, meanhaz, ci.lb,  ci.ub)) %>%
  ...
  ######################################################################
  calc_ip = function(d, age_list, severe){
    ip.data <- summary.stunt.incprop(d, agelist = age_list, severe.stunted = severe, method = calc_method)
    ip.region <- d %>% group_by(region) %>% do(summary.stunt.incprop(., agelist = age_list, severe.stunted = severe, method = calc_method)$ip.res)
    ip.cohort <-
      ip.data$ip.cohort %>% 
  ...
  calc_ci = function(d, age_list, birth_strat, severe){
    ci.data <- summary.ci(d, birthstrat = birth_strat, agelist = age_list, severe.stunted = severe, method = calc_method)
    ci.region <- d %>% group_by(region) %>% do(summary.ci(., agelist = age_list,  birthstrat = birth_strat, severe.stunted = severe, method = calc_method)$ci.res)
    ci.cohort <-
