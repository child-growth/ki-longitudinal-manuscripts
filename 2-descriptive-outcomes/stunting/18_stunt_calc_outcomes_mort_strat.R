...
mort <- readRDS(paste0(ghapdata_dir,"mortality.rds")) %>% subset(., select = c(studyid,country,subjid,dead))
head(mort)
...
d <- d %>% filter(!is.na(dead)) %>% droplevels()
d$dead <- factor(d$dead)
...
# calc_method = "REML"
# output_file_suffix = ""
# pooling_variable="dead"
res = calc_outcomes(data = d, calc_method = "REML", pooling_variable="dead", output_file_suffix = "")
res <- res %>% rename(dead=country_cat)
table(res$agecat)

saveRDS(res, file = paste0(res_dir,"stunting/stunt_mort_strat_pool.RDS"))
