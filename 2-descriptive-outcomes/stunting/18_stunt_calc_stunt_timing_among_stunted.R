...
colnames(d)
d <- d %>% select(studyid, country, region, subjid, agedays, haz)
d3 <- calc.ci.agecat(d, range = 3, birth="yes")

dstunt <- d3 %>% filter(agedays < 24 * 30.4167) %>% 
      group_by(studyid,country,subjid) %>%
...
cuminc3 = calc_ci(dstunt, age_list=agelst3, birth_strat = FALSE, severe = FALSE)
res <- cuminc3$ci.data$ci.res
res

res2 <- res %>% select(agecat, nstudy.f, nmeas.f, est, lb, ub)
