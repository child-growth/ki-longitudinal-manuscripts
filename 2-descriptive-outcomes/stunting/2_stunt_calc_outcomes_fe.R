...
#----------------------------------------
prev.data <- summary.prev.haz(dprev, method = "FE")
prev.region <- dprev  %>% group_by(region) %>% do(summary.prev.haz(., method = "FE")$prev.res)
prev.cohort <-
...
#----------------------------------------
sev.prev.data <- summary.prev.haz(dprev, severe.stunted = T, method = "FE")
sev.prev.region <-
  dprev  %>% group_by(region) %>% do(summary.prev.haz(., severe.stunted = T, method = "FE")$prev.res)
sev.prev.cohort <-
...
# mean haz
#----------------------------------------
haz.data <- summary.haz(dprev, method = "FE")
haz.region <- dprev %>% group_by(region) %>% do(summary.haz(., method = "FE")$haz.res)
haz.cohort <-
...
haz.data.vel <- summary.haz.age.sex(d_vel, method = "FE")
haz.region.vel <-  d_vel  %>% group_by(region) %>% do(summary.haz.age.sex(., method = "FE")$haz.res)
haz.cohort.vel <-
...
#----------------------------------------
dmon <- calc.monthly.agecat(d)
monthly.haz.data <- summary.haz(dmon, method = "FE")
monthly.haz.region <-  dmon  %>% group_by(region) %>% do(summary.haz(., method = "FE")$haz.res)
monthly.haz.cohort <-
...
#----------------------------------------
ip.data3 <- summary.stunt.incprop(d3, agelist = agelst3, severe.stunted = F, method = "FE")
ip.region3 <- d3 %>% group_by(region) %>% do(summary.stunt.incprop(., agelist = agelst3, method = "FE")$ip.res)
ip.cohort3 <-
  ip.data3$ip.cohort %>% 
...
# stratify by birth
#----------------------------------------
ip.data3.birthstrat <- summary.stunt.incprop(d3_birthstrat, agelist = agelst3_birthstrat, severe.stunted = F, method = "FE")
ip.region3.birthstrat <- d3_birthstrat %>% group_by(region) %>% do(summary.stunt.incprop(., agelist = agelst3_birthstrat, method = "FE")$ip.res)
...
#----------------------------------------
ip.data6 <- summary.stunt.incprop(d6, agelist = agelst6, severe.stunted = F, method = "FE")
ip.region6 <- d6  %>% group_by(region)%>% do(summary.stunt.incprop(., agelist = agelst6, method = "FE")$ip.res)
ip.cohort6 <-
  ip.data6$ip.cohort %>%
  subset(., select = c(cohort, region, agecat, nchild,  yi,  ci.lb,  ci.ub)) %>%
  rename(est = yi,  lb = ci.lb,  ub = ci.ub, nmeas=nchild)
...
# 3 month interval
#----------------------------------------
sev.ip.data3 <- summary.stunt.incprop(d3, agelist = agelst3, severe.stunted = T, method = "FE")
sev.ip.region3 <- d3 %>% group_by(region) %>% do(summary.stunt.incprop(., agelist = agelst3, severe.stunted = T, method = "FE")$ip.res)
sev.ip.cohort3 <-
...
# 6 month interval
#----------------------------------------
sev.ip.data6 <- summary.stunt.incprop(d6, agelist = agelst6, severe.stunted = T, method = "FE")
sev.ip.region6 <- d6 %>% group_by(region) %>% do(summary.stunt.incprop(., agelist = agelst6, severe.stunted = T, method = "FE")$ip.res)
sev.ip.cohort6 <-
...
#----------------------------------------
ci.data3 <- summary.ci(d3, agelist = agelst3, method = "FE")
ci.region3 <- d3 %>% group_by(region) %>% do(summary.ci(., agelist = agelst3, method = "FE")$ci.res)
ci.cohort3 <-
...
# stratify by birth 
#----------------------------------------
ci.data3.birthstrat <- summary.ci(d3_birthstrat, birthstrat=TRUE, agelist = agelst3_birthstrat, method = "FE")
ci.region3.birthstrat <- d3_birthstrat %>% group_by(region) %>%
  do(summary.ci(., agelist = agelst3_birthstrat,  birthstrat=TRUE, severe.stunted=FALSE, method = "FE")$ci.res)
...
#----------------------------------------
ci.data6 <- summary.ci(d6, agelist = agelst6, method = "FE")
ci.region6 <- d6 %>% group_by(region) %>% do(summary.ci(., agelist = agelst6, method = "FE")$ci.res)
ci.cohort6 <-
...
# severe
#----------------------------------------
sev.ci.data3 <- summary.ci(d3, agelist = agelst3, severe.stunted = T, method = "FE")
sev.ci.region3 <- d3 %>% group_by(region) %>% do(summary.ci(., agelist = agelst3, severe.stunted = T, method = "FE")$ci.res)
sev.ci.cohort3 <-
...
# severe
#----------------------------------------
sev.ci.data6 <- summary.ci(d6, agelist = agelst6, method = "FE")
sev.ci.region6 <- d6 %>% group_by(region) %>% do(summary.ci(., agelist = agelst6, severe.stunted = T, method = "FE")$ci.res)
sev.ci.cohort6 <-
