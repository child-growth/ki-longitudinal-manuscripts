...
ci.data3 <- summary.ci(d3, age.range=3)
ci.region3 <- d3 %>% group_by(region) %>% do(summary.ci(., age.range=3)$ci.res)
ci.country3 <- d3 %>% group_by(country) %>% do(summary.ci(., age.range=3)$ci.res) %>% rename(region=country)
ci.cohort3 <-
...
ci.data.nobirth <- summary.ci(d_noBW, age.range=6)
ci.region.nobirth <- d %>% group_by(region) %>% do(summary.ci(., age.range=6)$ci.res)
ci.country.nobirth <- d %>% group_by(country) %>% do(summary.ci(., age.range=6)$ci.res) %>% rename(region=country)
ci.cohort.nobirth <-
...
ci.data.nobirth3 <- summary.ci(d3, age.range=3)
ci.region.nobirth3 <- d3 %>% group_by(region) %>% do(summary.ci(., age.range=3)$ci.res)
ci.country.nobirth3 <- d3 %>% group_by(country) %>% do(summary.ci(., age.range=3)$ci.res) %>% rename(region=country)
ci.cohort.nobirth3 <-
...
d <- calc.ci.agecat(d, range = 6)
agelst = list("0-6 months", "6-12 months", "12-18 months", "18-24 months")
sev.ci.data <- summary.ci(d=d,  severe.wasted = T, age.range=6)
sev.ci.region <- d %>% group_by(region) %>% do(summary.ci(., severe.wasted = T, age.range=6)$ci.res)
sev.ci.country <- d %>% group_by(country) %>% do(summary.ci(., severe.wasted = T, age.range=6)$ci.res) %>% rename(region=country)
sev.ci.cohort <-
...
ir.country.nobirth <- d_noBW %>% group_by(country) %>% do(summary.ir(.)$ir.res) %>% rename(region=country)
ir.cohort.nobirth <-
  ir.data$ir.cohort %>% subset(., select = c(cohort, region, agecat,  yi,  ci.lb,  ci.ub)) %>%
  rename(est = yi,  lb = ci.lb,  ub = ci.ub)
...
sev.ir.country <- d %>% group_by(country) %>% do(summary.ir(., sev.wasting = T)$ir.res) %>% rename(region=country)
sev.ir.cohort <-
