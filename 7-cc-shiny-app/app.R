...
#Plot parameters
yticks <- c(0.25, 0.50, 1.00, 2.00, 4.00, 8.00)
...
ui <- navbarPage("HBGDki Results Dashboard",
          tabPanel("Exposure analysis",
              titlePanel("Associations between child, parental, and household characteristics and growth faltering outcomes"),
          ...
          tabPanel("Splines",
             fluidRow(
               ...
               column(4, selectInput("spline_exposure",
                                     "Exposure Variable:",
                                     choices = levels(droplevels(df$intervention_variable))))
             ),
...
input <- data.frame(exposure = "HH wealth", outcome = "Prevalence of stunting", age = "6 months", parameter = "Relative Risk", region = "All", adjusted = "Yes")
