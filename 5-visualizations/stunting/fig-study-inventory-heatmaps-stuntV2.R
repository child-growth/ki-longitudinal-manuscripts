...
shorten_descriptions<-function(dd){
  ...
  dd$short_description[dd$study_id=='XXXX'] <- 'Vit. A Sup., Serrinha'
  dd$short_description[dd$study_id=='XXXX'] <- 'Resp. Pathogens'
...
hm <- ggplot(dd,aes(x=agecat,y=studycountry)) +
...
#-----------------------------------
stpbar <- sidebar +
  aes(y=overall_stuntprev,fill=region) +
...
# n by age top plot 
#-----------------------------------
nagebar <- ggplot(dp, aes(y = N/1000, x = age)) +
