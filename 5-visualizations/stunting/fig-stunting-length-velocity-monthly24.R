...
plot_laz <- ggplot(velplot_laz, aes(y=Mean,x=strata))+
  ...
  scale_color_manual(values=c(tableau10[4],tableau10[1]))+  
  scale_y_continuous(limits=c(-0.25,0.2), breaks=seq(-0.25,0.2,0.05), labels=seq(-0.25,0.2,0.05)) +
  xlab("Child age, months") +  
...
ggsave(plot_laz, file=paste0("figures/stunting/fig-",plot_laz_name,".png"), width=9, height=3)
...
# save plot and underlying data
ggsave(plot_laz_strat, file=paste0("figures/stunting/fig-",plot_laz_strat_name,".png"), width=18, height=4)
...
plot_cm <- ggplot(velplot_cm, aes(y = length_cm, x = strata)) +
  ...
  scale_y_continuous(limits=c(0.5,4), breaks=seq(0,4,0.25), labels=seq(0,4,0.25)) +
  xlab("Child age, months") +  
...
plot_cm_strat <- ggplot(velplot_cm_strat, aes(y=Mean,x=strata))+
  ...
  geom_linerange(aes(ymin=Lower.95.CI, ymax=Upper.95.CI, color=sex),
                 alpha=0.5, size = 1) +
  scale_color_manual(values=c(tableau10[4],tableau10[1]))+  
  scale_y_continuous(limits=c(0.2,4.25), breaks=seq(0.2,4.25,0.25), labels=seq(0.2,4.25,0.25)) +
  xlab("Child age, months") +  
  ylab("Difference in length (cm) per month")+
