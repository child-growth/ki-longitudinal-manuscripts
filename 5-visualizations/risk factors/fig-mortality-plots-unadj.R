...
#Load data
dfull <- readRDS(paste0(here::here(),"/results/rf results/raw longbow results/unadj_mortality_2020-03-08.RDS"))
head(dfull)
...
#Save cleaned data
saveRDS(RMAest, paste0(here::here(),"/results/rf results/pooled_mortality_RR_unadj_results.rds"))
...
pmort <- ggplot(d2[d2$outcome_variable=="Relative risk of\nmortality by 24 months",], aes(x=outcome_label)) +
  ...
  labs(y = "Relative risk", x = "Exposure 0-6 months") +
  ...
  ggtitle("Unadjusted associations between growth\nfailure under 6mo and mortality") + 
  theme(plot.title = element_text(size = 12, face = "bold")) +
  coord_flip(ylim=c(1,9))
#expand=c(0,0)) 


ggsave(pmort, file=here("/figures/risk-factor/fig-mort-RR-unadj.png"), width=5.2, height=5.2)
