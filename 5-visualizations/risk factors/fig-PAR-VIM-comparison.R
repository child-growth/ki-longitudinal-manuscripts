...
dhaz$outlier <- ifelse(is_outlier(dhaz$PAR-dhaz$VIM, thres=5), as.character(dhaz$RFlabel.x),NA)
dwhz$outlier <- ifelse(is_outlier(dwhz$PAR-dwhz$VIM, thres=2), as.character(dwhz$RFlabel.x),NA)
...
pVIMhaz <- ggplot(dhaz, aes(x=-PAR, y=-VIM)) + 
  ...
  geom_text_repel(aes(label = outlier), na.rm = TRUE, size = 2.5, nudge_y = .1, box.padding = 1.5, color="grey40", direction = "both", force=20, ylim = c(0.18, 0.2)#, xlim = c(0, 0.25)#, ylim = c(0.18, 0.4)
                 )+
...
pVIMwhz <- ggplot(dwhz, aes(x=-PAR, y=-VIM)) + 
  geom_point(size = 2, alpha=0.5) +
  coord_fixed(xlim = c(-0.1,0.2), ylim = c(-0.1,0.2)) +
  labs(x = "Attributable difference\nin WLZ, fixed reference", y = "Attributable difference\nin WLZ, optimal intervention") +
  geom_abline(slope=1,intercept=0) +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_hline(yintercept = 0, linetype="dashed") +
  geom_text_repel(aes(label = outlier), na.rm = TRUE,  force=12, size = 2.5, xlim=c(0.1, 10), color="grey40"#, ylim = c(-10,0.5)
                  ) +
...
ggsave(pVIMhaz, file=paste0("C:/Users/andre/Documents/HBGDki/ki-longitudinal-manuscripts/figures/risk factor/fig-VIM-PAR-comp-haz.png"), height=4, width=4)
ggsave(pVIMwhz, file=paste0("C:/Users/andre/Documents/HBGDki/ki-longitudinal-manuscripts/figures/risk factor/fig-VIM-PAR-comp-whz.png"), height=4, width=4)
