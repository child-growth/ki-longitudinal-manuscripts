...
d <- d %>% filter(!(outcome_variable %in% c("dead0plus","dead624","dead6plus"))) %>%
  mutate(outcome_variable=factor(outcome_variable, levels= c("stunted","wasted","sstunted","swasted","ever_wasted","ever_stunted","ever_sstunted","ever_swasted","ever_co","pers_wast","wast_rec90d")))
...
p <- ggplot(d2, aes(x=intervention_level)) +
  geom_point(aes(y=RR, color=contrast, shape=ref)) +
  ...
  ggtitle(d2$intervention_variable[1])

ggsave(p, file=here("figures/risk-factor/RR-plots/fig-RR-test.png"), width=14, height=5.2)

# To do:
#   ggtitle for exposure variable
# drop "Outcome: " from facet labels
...
#res <- d %>% group_by(intervention_variable) %>% do(res=RR_plot(.))
