...
prev <- prev %>% subset(., select = -c(CI1, CI2)) %>% mutate(exp_prev = 1 - prev)
  
dim(RMAest_raw)
df <- left_join(RMAest_raw, prev, by = c("outcome_variable","intervention_variable", "agecat","intervention_level","region"))
dim(df)

#temp
df <- df %>% filter(!is.na(prev))

test <- df$PAR / df$exp_prev * 100
summary(test)
