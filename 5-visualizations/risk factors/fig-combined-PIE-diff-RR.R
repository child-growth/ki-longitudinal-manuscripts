...
# CIR
CIR_raw <- readRDS(paste0(BV_dir,"/results/rf results/pooled_RR_results.rds")) %>% mutate(parameter="CIR") %>% filter(intervention_variable!="perdiar24")
...
df_full <- bind_rows(par_raw, CIR_raw, ATE_raw)
...
df <- df %>%
  mutate(
    ...
    outcome_variable = factor(outcome_variable, levels=c("haz","ever_stunted","whz","ever_wasted" )))
