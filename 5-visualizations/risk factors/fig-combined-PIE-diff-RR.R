...
df_CIR <- CIR %>% subset(., select = c(outcome_variable, intervention_variable, RR, RR.CI1, RR.CI2, RFlabel, RFlabel_ref,  RFtype)) %>% 
  filter(!is.na(RR)) %>% 
  filter(RR.CI1 !=RR.CI2) %>% #drop reference levels
  mutate(parameter="CIR")
...
pCIR_stunt <- ggplot(plotdf_CIR %>% filter(outcome_variable=="LAZ"), aes(x=RFlabel_ref, group=RFgroup, color=RFgroup)) + 
