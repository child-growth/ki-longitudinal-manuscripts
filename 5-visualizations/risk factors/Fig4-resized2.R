...
p2 <- splines[[2]]  + ggtitle("") + 
  theme(legend.position = pos2, legend.spacing.y = unit(0.5, 'mm'),  legend.title=element_text(size=10), legend.text=element_text(size=8)) + 
  ...
  theme(legend.key = element_blank(), legend.background = element_blank(), legend.box.background = element_rect(color = NA)) 
p4 <- splines[[4]]  + ggtitle("") +  
  theme(legend.position = pos, legend.spacing.y = unit(0.5, 'mm'),  legend.title=element_text(size=10), legend.text=element_text(size=8)) +
  guides(color = guide_legend("Maternal\nheight", nrow=3)) + scale_x_continuous(limits=c(0,730), expand = c(0, 0), breaks = 0:6*30.41*4, labels = c(0, seq(4, 24, 4))) + 
  theme(legend.key = element_blank(), legend.background = element_blank(),legend.box.background = element_rect(color = NA))  +scale_y_continuous(limits=c(-2.4, -0.6), breaks = seq(-2.4, 0.4, 0.2), labels = round(seq(-2.4, 0.4, 0.2),1)) 
