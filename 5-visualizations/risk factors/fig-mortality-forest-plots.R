...
dfull <- dfull %>% filter(type=="RR", adjusted==1)
dfull <- dfull %>% filter(outcome_variable=="dead") %>% 
  filter(ci_lower != ci_upper, agecat=="0-24 months") %>%
  mutate(pooled=0) %>%
  rename(RR=estimate, RR.CI1=ci_lower, RR.CI2=ci_upper)
dfull <- RMA_clean(dfull)

dRE <- readRDS(paste0(here::here(),"/results/rf results/pooled_mortality_RR_results.rds"))
dFE <- readRDS(paste0(here::here(),"/results/rf results/pooled_mortality_RR_sens_results.rds")) %>% filter( analysis=="Fixed Effects")
