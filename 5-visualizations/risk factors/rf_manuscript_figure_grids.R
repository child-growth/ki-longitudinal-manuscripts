...
# #Figure 4 (created in new script of the right size)
...
fig5a <- plot_grid(plen_lagwhz, p_earlywast, labels = "auto", ncol = 2, align = 'v', axis = 'l', rel_widths=c(2,1))


pmort_comb = plot_grid(pmort[[2]], pmort[[1]], labels = c("c","d"),  nrow = 1, align = 'v', axis = 'l', rel_widths = c(1.5, 1))
fig5  <- plot_grid(fig5a, pmort_comb, labels = c("",""), ncol = 1, rel_heights = c(1,1), align = 'h', axis = 'l')

ggsave(fig5, file=paste0(BV_dir,"/figures/manuscript-figure-composites/risk-factor/fig5.png"), width=18.3/1.25, height=9/1.25)
ggsave(plot = fig5, filename=paste0(here::here(),"/figures/manuscript-pdfs/c&c/Fig5.pdf"), device='pdf', width=18.3/1.25, height=9/1.25)
#ggsave(plot = fig5, filename=paste0(here::here(),"/figures/manuscript-pdfs/c&c/Fig5_resized.pdf"), units="cm", device="pdf", width=17, height=17)
