...
#Figure for mortality analysis, only including children who have measures within
...
#Load data
dfull <- readRDS(paste0(here::here(),"/results/rf results/raw longbow results/mortality_sens_2020-05-02.RDS"))
head(dfull)
...
#Save cleaned data
saveRDS(RMAest, paste0(here::here(),"/results/rf results/pooled_mortality_RR_results_sens_birth.rds"))


#load primary cleaned data
RMAest_main <- readRDS(paste0(here::here(),"/results/rf results/pooled_mortality_RR_results.rds")) %>%
  filter(outcome_variable=="dead", analysis=="Random Effects") %>% subset(., select = -c(analysis))


d <- bind_rows(
  data.frame(RMAest_main, Analysis="Primary"), 
  data.frame(RMAest, Analysis="Sensitivity"))
d <- d %>% filter()
d <- droplevels(d)
...
d2$Analysis <- factor(d2$Analysis, levels=rev(c("Primary","Sensitivity")))

pmort <- ggplot(d2[d2$outcome_variable=="Relative risk of\nmortality by 24 months",], aes(x=outcome_label, group=Analysis)) +
  geom_point(aes(y=RR, color=Measure, shape=Analysis), size=3, stroke = 1.5, position = position_dodge(width=0.5)) +
  geom_linerange(aes(ymin=RR.CI1, ymax=RR.CI2, color=Measure), position = position_dodge(width=0.5)) +
  ...
  theme(strip.placement = "outside",
        ...
        plot.title = element_text(hjust = 0.5, lineheight=.4),
        strip.background = element_blank(),
        text = element_text(size=16), 
        legend.position = "bottom") + 
  # ggtitle("Sensitivity analysis on the effect of\nexcluding children without
  # anthropometry measures\nin the first month of life on
  #         estimated\nassociations between growth failure under 6mo and mortality") + 
  theme(plot.title = element_text(size = 12, face = "bold")) +
  coord_flip(ylim=c(1,9)) +
  guides(fill=FALSE, colour=FALSE, shape=guide_legend(reverse=TRUE))
pmort

ggsave(pmort, file=here("/figures/risk-factor/fig-mort-RR-sens_birth.png"), width=5.2, height=5.2)

#Save plot object
saveRDS(pmort, file=here("results/rf_mort-sens_plot_birth_object.RDS"))
