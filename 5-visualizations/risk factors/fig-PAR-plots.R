...
df$outcome_variable <- gsub("pers_wast", "Persistently wasted", df$outcome_variable)
  ...
  for(j in unique(df$outcome_variable)){
    for(k in unique(df$agecat)){
      ...
      ppar <-  ggplot(dpool, aes(x=reorder(RFlabel, PAR))) + 
        ...
        labs(x = "Exposure", y = "Attributable Risk (%)") +
        ...
        ggtitle(paste0("Attributable risk (%)\n", dpool$outcome_variable[1]," - ", dpool$agecat[1],", ", dpool$region[1])) + 
        guides(color=FALSE, shape=FALSE)
      
      ggsave(ppar, file=paste0("C:/Users/andre/Documents/HBGDki/ki-longitudinal-manuscripts/figures/risk factor/PAR/fig-",dpool$region[1], "-", dpool$outcome_variable[1], "-", gsub(" ","",dpool$agecat[1]), "-PAR.png"), height=10, width=8)
    }    
  }
}
