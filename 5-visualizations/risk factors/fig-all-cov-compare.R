...
#Load data
results <- readRDS(paste0(res_dir, "rf results/longbow results/results_full_cov.RDS"))
...
#check some estimates
par <- d %>% filter(intervention_variable=="fhtcm", outcome_variable=="haz", agecat=="24 months")
head(par)
par %>% group_by(intervention_variable, agecat, intervention_level, baseline_level, outcome_variable) %>%
...
#load 2-step results 
fullPAR <- readRDS(paste0(BV_dir,"/results/rf results/pooled_Zscore_PAR_results.rds"))
...
ggsave(pPAR, file=paste0(BV_dir, "/figures/risk-factor/fig-PAR-all_cov.png"), height=10, width=16)
