...
df <- d %>% filter(agedays < 31, measurefreq!="yearly")

p1 <- ggplot(df, aes(x=agedays)) + geom_histogram() 
p2 <- ggplot(df, aes(x=agedays)) + geom_histogram() + facet_wrap(~studyid, scales = "free")
