...
md <- md[grepl("IND", md$country) & !is.na(md$measurefreq),]
wmd <- wmd[grepl("IND", wmd$country) & !is.na(wmd$measurefreq),]
md <- md %>% filter((measurefreq!="yearly" & !is.na(measurefreq)) | study_id=="VITAMIN-A")
wmd <- wmd %>% filter(measurefreq!="yearly" & !is.na(measurefreq) | study_id=="VITAMIN-A")
...
dd <- md %>% filter(countrycohort=="INDIA")
...
dp$wpcat <- cut(dp$wp,breaks=c(0,5,10,15,20,25,30, 35,40,100),labels=c("<5","5-10","10-15","15-20","20-25","25-30","30-35","35-40",">40"))
...
dp <- dp %>% 
  group_by(study_id) %>% mutate(Nfreq=sum(!is.na(whzcat)), Nobs=max(nobs, na.rm=T)) %>% ungroup() %>%
  dplyr::arrange(Nfreq, desc(Nobs))
...
# dd <- dd %>% 
#   group_by(study_id) %>% mutate(Nfreq=n(),Nobs=max(nobs, na.rm=T)) %>% ungroup() %>%
#   dplyr::arrange(Nfreq, Nobs)
dd$studycountry <- sapply(dd$studycountry, function(x) as.character(x))
dd$studycountry <- factor(dd$studycountry, levels = levels(dp$studycountry))
...
hm <- ggplot(dp,aes(x=age,y=studycountry)) +
  ...
  #facet_grid(measurefreq~., scales = "free_y", space="free") +
...
wastphm <- hm + 
  ...
  scale_fill_viridis(na.value="grey90", option = "C", discrete = T,
                     ...
                     guide=guide_legend(title="Wasting Prevalence",title.vjust = 1,
                                        label.position="bottom",label.hjust=0.5,nrow=1))
...
wastphm2 = whzhm + theme(plot.margin = unit(c(0,0.25,0.25,0.25), "cm"))
wpbar2 = wpbar + theme(plot.margin = unit(c(1.575,0.3,1.825,0.1), "cm"))
nbar2 = nbar + theme(plot.margin = unit(c(1.575,0.25,1.825,0.1), "cm"))
nagebar2 = nagebar + theme(plot.margin = unit(c(0.25,2.9,0,6.75), "cm"))
empty <- grid::textGrob("") 
...
ggsave(wastphm2, filename=paste0(here(),"/figures/India/fig-India-cohorts-heatmap.png"),width=w, height=h)
ggsave(wastphm, filename=paste0(here(),"/figures/India/fig-India-cohorts-wast-prev-heatmap.png"),width=w, height=h)
ggsave(wpbar2, filename=paste0(here(),"/figures/India/fig-India-cohorts-wasting-prev-bar.png"),width=h2, height=h)
ggsave(nbar2, filename=paste0(here(),"/figures/India/fig-India-cohorts-N-bar.png"),width=h2,height=h)
ggsave(nagebar2, filename=paste0(here(),"/figures/India/fig-India-cohorts-N-age-bar.png"),width=w,height=h2)


awstpgrid <- grid.arrange(nagebar2,empty, empty,
                          whzhm, nbar, wpbar, nrow = 2, ncol = 3,
                          heights = c(25,100),
                          widths=c(100,20,20))


# save plot and underlying data
ggsave(filename=paste0(here(),"/figures/India/fig-India-cohorts-heatmap.pdf"),
       plot = awstpgrid,device='pdf',width=11.5,height=6)
...
# heat map plot scheme
hm <- ggplot(dp[dp$study_id!="VITAMIN-A",],aes(x=age,y=studycountry)) +
  # facet over measurement frequency
  #facet_grid(region~. ) + #,scales='free_y',space='free_y') +
  #facet_grid(measurefreq~., scales = "free_y", space="free") +
  #add border white colour of line thickness 0.25
...
# side bar plot scheme
sidebar <- ggplot(data = dd[dd$study_id!="VITAMIN-A",], aes(x = studycountry)) + 
  geom_bar(stat = "identity") +
  coord_flip() + 
  facet_grid(region~.,scales='free_y',space='free_y') +
  #remove extra space
...
# n by age top plot 
#-----------------------------------
nagebar <- ggplot(dp[dp$study_id!="VITAMIN-A",], aes(y = nobs/1000, x = age)) +
  geom_bar(stat = "identity", fill='gray70') +  
  scale_x_continuous(breaks = seq(0,24,1), labels = seq(0,24,1)) +
  theme(
    # make background white
...
ggsave(wastphm2, filename=paste0(here(),"/figures/India/fig-India-cohorts-heatmap_quart.png"),width=w, height=h)
ggsave(wastphm, filename=paste0(here(),"/figures/India/fig-India-cohorts-wast-prev-heatmap_quart.png"),width=w, height=h)
ggsave(wpbar2, filename=paste0(here(),"/figures/India/fig-India-cohorts-wasting-prev-bar_quart.png"),width=h2, height=h)
ggsave(nbar2, filename=paste0(here(),"/figures/India/fig-India-cohorts-N-bar_quart.png"),width=h2,height=h)
ggsave(nagebar2, filename=paste0(here(),"/figures/India/fig-India-cohorts-N-age-bar_quart.png"),width=w,height=h2)




awstpgrid2 <- grid.arrange(nagebar2,empty, empty,
                           whzhm, nbar, wpbar, nrow = 2, ncol = 3,
                           heights = c(25,100),
                           widths=c(100,20,20))
...
# save plot and underlying data
ggsave(filename=paste0(here(),"/figures/India/fig-India-cohorts-heatmap_quarterly.pdf"),
       plot = awstpgrid2,device='pdf',width=11.5,height=6)
