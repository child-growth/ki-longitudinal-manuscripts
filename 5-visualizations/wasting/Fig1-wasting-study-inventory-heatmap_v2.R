...
hm <- ggplot(dd,aes(x=as.numeric(agecat),y=studycountry)) +
  ...
  theme(
    aspect.ratio = 1,
    # legend options
    legend.title=element_text(color=textcol,size=6),
    #reduce/remove legend margin
    legend.margin = margin(grid::unit(0,"cm")),
    #change legend text properties
    legend.text=element_text(size=5),
    #change legend key height
    legend.key.height=grid::unit(0.15,"cm"),
    #set a slim legend
    legend.key.width=grid::unit(0.05,"cm"),
    #legend.key.size = unit(0.25, "cm"),
    #legend.spacing.x = unit(0, 'cm'),
    #move legend to the bottom
    legend.position = "bottom",
    #set x axis text size and colour
    axis.text.x=element_text(size=5,colour=textcol,angle=0,vjust=0.5),
    axis.title.x = element_text(size=6),
    #set y axis text colour and adjust vertical justification
    axis.text.y=element_text(size=5,vjust = 0.5,colour=textcol),
    #change axis ticks thickness
    axis.ticks=element_line(size=0.4),
    # axis.ticks.x=element_blank(),
    #change title font, size, colour and justification
    plot.title=element_text(colour=textcol,hjust=0,size=8,face="bold"),
    #format facet labels
...
nagebar <- ggplot(dd, aes(y = N/1000, x = as.numeric(agecat))) +
  ...
  theme(
    ...
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
    plot.title=element_text(colour=textcol,hjust=0.04,size=8,face="bold"),
    # remove x axis ticks
...
nagebar2 = nagebar + theme(plot.margin = unit(c(0.15, 0.46,-0.55, 3.56), "cm"))
wastphm2 = wastphm + theme(plot.margin = unit(c(0, 0.5, 0, 1), "cm")) #t = 0, r = 0, b = 0, l = 0

nbar2 = nbar + theme(plot.margin = unit(c(.55,0.25,.61,0.1), "cm"))
wpbar2 = wpbar + theme(plot.margin = unit(c(.55,0.3,.61,0.1), "cm"))
...
ggsave(filename=paste0(here::here(),"/figures/manuscript-pdfs/wasting/Fig1_resized.pdf"), plot = awstpgrid,device='pdf',width=18,height=12, units="cm")
