...
######################################
# Restrict to only South Asia
######################################
dd = dd %>% filter(region == "Asia")
colnames(dd)

# gather N measurements by month data into long format
dnsubj <- select(dd,study_id,country,studycountry,region,stuntprev,wastprev,starts_with('n'), measurefreq) %>%
  select(-neurocog_data,-nutrition,-notes,-num_countries,-numcountry,-numsubj,-numobs,-nmeas, -numsubj_u24mo, -numobs_u24mo) %>%
  gather(age,nobs,-study_id,-country,-studycountry,-region,-stuntprev, -wastprev, -measurefreq) %>%
  mutate(age=as.integer(str_sub(age,2,-1)),nobs=as.integer(nobs)) %>%
  select(study_id,country,studycountry,region,stuntprev,wastprev,age,nobs, measurefreq) %>%
  filter(age>=0 & age <=24 ) %>%
  arrange(region,stuntprev,wastprev) 

# gather stunting prev by month data into long format
dstuntp <- select(dd,study_id,country,studycountry,starts_with('stuntprev_m'), measurefreq) %>%
  gather(age,stp,-study_id,-country,-studycountry, -measurefreq) %>%
  mutate(age=as.integer(str_sub(age,12,-1))) %>%
  select(study_id,country,studycountry,age,stp, measurefreq) %>%
  filter(age>=0 & age <=24 )

# gather wasting prev by month data into long format
dwastp <- select(dd,study_id,country,studycountry,starts_with('wastprev_m'), measurefreq) %>%
  gather(age,wp,-study_id,-country,-studycountry, -measurefreq) %>%
  mutate(age=as.integer(str_sub(age,11,-1))) %>%
  select(study_id,country,studycountry,age,wp, measurefreq) %>%
  filter(age>=0 & age <=24 )

# gather meanHAZ by month data into long format
dhaz <- select(dd,study_id,country,studycountry,starts_with('meanHAZ_m'), measurefreq) %>%
  gather(age,haz,-study_id,-country,-studycountry, -measurefreq) %>%
  mutate(age=as.integer(str_sub(age,10,-1))) %>%
  select(study_id,country,studycountry,age,haz, measurefreq) %>%
  filter(age>=0 & age <=24 )

# gather meanWHZ by month data into long format
dwhz <- select(dd,study_id,country,studycountry,starts_with('meanWHZ_m'), measurefreq) %>%
  gather(age,whz,-study_id,-country,-studycountry, -measurefreq) %>%
  mutate(age=as.integer(str_sub(age,10,-1))) %>%
  select(study_id,country,studycountry,age,whz, measurefreq) %>%
  filter(age>=0 & age <=24 )
...
dim(dnsubj)
dim(dstuntp)
dp <- left_join(dnsubj,dstuntp,by=c('study_id','studycountry','country','age', 'measurefreq'))
dim(dp)
dp <- left_join(dp,dwastp,by=c('study_id','studycountry','country','age', 'measurefreq'))
dim(dp)
dp <- left_join(dp,dhaz,by=c('study_id','studycountry','country','age', 'measurefreq'))
dim(dp)
dp <- left_join(dp,dwhz,by=c('study_id','studycountry','country','age', 'measurefreq'))
dim(dp)
...
sidebar <- ggplot(data = dd, aes(x = studycountry)) + 
  ...
  coord_flip() + 
  facet_grid(measurefreq~.,scales='free_y',space='free_y') +
  #remove extra space
...
nagebar <- ggplot(dp, aes(y = nobs/1000, x = age)) +
  ...
  scale_y_continuous(expand=c(0,0),limits=c(0,35),
                     breaks=seq(0,35,by=2),labels=seq(0,35,by=2))+
  ylab("Sample size (1000s)") + xlab("") +
  geom_hline(yintercept = seq(0,35,by=2),color='white',size=0.3) +
  ggtitle("a")
