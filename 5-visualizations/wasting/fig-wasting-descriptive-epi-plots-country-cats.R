...
decade_data <- readRDS(paste0(res_dir,"wasting/wast_decade_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "90s", "2000s", "2010s"
  ))) 

gdp_data <- readRDS(paste0(res_dir,"wasting/wast_gdp_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "Low income", "Middle income"
  )))

gdi_data <- readRDS(paste0(res_dir, "wasting/wast_gdi_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "Very-low", "Low", "Medium"
  )))

gii_data <- readRDS(paste0(res_dir, "wasting/wast_gii_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "Low", "Medium", "High"
  )))

chi_data <- readRDS(paste0(res_dir, "wasting/wast_chi_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "Low", "Medium", "High"
  )))

gini_data <- readRDS(paste0(res_dir, "wasting/wast_gini_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "Low", "Medium", "High"
  )))

he_data <- readRDS(paste0(res_dir, "wasting/wast_he_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "Low", "Medium", "High"
  )))

pov_data <- readRDS(paste0(res_dir, "wasting/wast_pov_pool.RDS")) %>% 
  mutate(country_cat = factor(country_cat, levels = c(
    "Overall", "Low", "Medium", "High"
  )))
...
# save plot and underlying data
ggsave(ip_plot_primary_pov$plot, file=paste0(fig_dir, "wasting/fig-",ip_plot_name_pov,".png"), width=16, povight=4)
