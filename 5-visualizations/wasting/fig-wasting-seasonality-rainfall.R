...
rain2 <- rain %>%
  gather(month,rain, -studyid, -country) 
rain2$month = gsub("_pre","",rain2$month)
rain2$month = factor(rain2$month, levels=c("Jan", "Feb", "Mar", "Apr", "May","Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))
head(rain2)


ggplot(rain2, aes(x=month, y=rain)) + geom_bar(stat='identity', width=1) + facet_wrap(~paste0(studyid, ", ", country)) +
  geom_smooth(data = d, aes(x=jday, y=whz, color=region), span=1, se=F, size=2)




#updates to add:
#order facets by seasonality index (or split into 3 plots)
#make a day variable in the rain2 dataset so that X-axes line up
...
dim(rain)
dim(d)
d <- left_join(d, rain, by = c("studyid","country"))
dim(d)
...
#
p1 <- ggplot(d[d$studyid=="ki1000108-IRC",], aes(x=jday, y=whz)) + geom_smooth(aes(color=region), span=1, se=F, size=2) +
  #geom_smooth(aes(group=cohort), color="grey20", span=1, se=F,linetype=3, size=1) + 
  geom_histogram(data = d_bg, fill = "grey") +
  xlab("Month of the year") + ylab("Mean WLZ") +
...
# ggsave(p1, file=paste0(here(),"/figures/wasting/fig-",p1_name,"-rain.png"), width=6, height=3)
# 
# 
# 
...
# 
# # save plot and underlying data
# ggsave(p2, file=paste0(here(),"/figures/wasting/fig-",p2_name,"-rain.png"), width=6, height=3)
# 
# 
# 
# 
...
# 
# # save plot and underlying data
# ggsave(p3, file=paste0(here(),"/figures/wasting/fig-",p3_name,"-rain.png"), width=8, height=5)
# 
# 
# 
...
# 
# # save plot and underlying data
# ggsave(p4, file=paste0(here(),"/figures/wasting/fig-",p4_name,"-rain.png"), width=8, height=5)
# 
# 
# #Save plot objects
