##########################################
# ki longitudinal manuscripts
# wasting analysis

# figure: stacked density of cohorts of 
# children who incovered at different ages. 
# histograms show distribution of LAZ
# at subsequent measurements following incidence.
##########################################

#-----------------------------------------
# wasting incidence density
#-----------------------------------------
rm(list=ls())
source(paste0(here::here(), "/0-config.R"))

# load wasting incidence cohort data
d = readRDS(paste0(res_dir, "wast_rec_cohort.RDS"))

d$subjid <- as.numeric(d$subjid)

# drop data beyond 16 months since it is 
# sparse in most studies
d = d %>% filter(agedays<=16 * 30.4167)

#-----------------------------------------
# function to filter data to children
# who incovered from wasting within 
# a specific age range 

# inputs:
# data = dataset created by 5_wast_inc_cohort.R
# age_upper = age of incidence 
#-----------------------------------------
get_inc = function(data, age_upper){
  age_lower = age_upper - 3
  age_range = paste0(age_lower, "-", age_upper, " months")
  
  # identify children who had an onset of wasting in 
  # this age range
  inc_data = data %>% 
    filter(inc_wasted == 1 & agecat == age_range) %>%
    select(studyid, country, subjid) %>%
    mutate(age_inc = age_range)
  
  inc_data = inc_data[!duplicated(inc_data),]
  
  inc_data_whz = full_join(inc_data, data, 
                           by = c("studyid", "country", "subjid")) %>%
    filter(!is.na(age_inc)) %>%
    select(studyid, country, subjid, agedays, whz, age_inc, born_wasted)
  
  # subsequent measurement ages after incidence
  age_meas = seq(age_upper, 15, 3) 
  
  
  inc_meas_sub_list = lapply(age_meas, function(x) 
    subset_inc(data = inc_data_whz, 
               age_months = x,
               age_range = age_range))
  
  
  inc_meas_sub = bind_rows(inc_meas_sub_list)
  
}


# --------------------------------------------
# function to subset incidence individual level 
# data to whz at a list of specific ages in months
# --------------------------------------------
subset_inc = function(data, age_months, age_range){
  out = data %>% 
    filter(agedays >= ((age_months-0.5)*30.4167) & 
             agedays <= ((age_months+0)*30.4167)) %>%
    mutate(age_meas = paste0(age_months, " month measurement"),
           age_inc = age_range)
  
  return(out)
}


age_range_list = as.list(seq(3, 12, 3))

inc_data = lapply(age_range_list, function(x)
  get_inc(
    data = d,
    age_upper = x
  ))


# bind list elements
plot_data = bind_rows(inc_data) 

summary = plot_data %>%
  group_by(age_inc, age_meas) %>%
  summarise(
    nchild = length(unique(subjid)),
    nstudy = length(unique(studyid)),
    ncountry = length(unique(country)),
    mean_whz= mean(whz)
  )


# --------------------------------------------
# density plot of kids who incovered
# --------------------------------------------
plot_data = plot_data %>%
  mutate(age_meas = factor(
    age_meas,
    levels =
      c(
        "15 month measurement",
        "12 month measurement",
        "9 month measurement",
        "6 month measurement",
        "3 month measurement"
      )
  ),
  age_inc = factor(
    age_inc,
    levels =
      c(
        "0-3 months",
        "3-6 months",
        "6-9 months",
        "9-12 months"
      )
  )
  )

# --------------------------------------------
# stacked histogram plot
# --------------------------------------------
# drop whz at tails of distributions
plot_data_sub = plot_data %>% filter(whz >=-5 & whz <=3.5)

# rename labels
plot_data_sub = plot_data_sub %>%
  mutate(age_inc_f = case_when(
    age_inc == "0-3 months" ~ "wasting incidence\nbefore 3 months",
    age_inc == "3-6 months" ~ "wasting incidence\nbefore 6 months",
    age_inc == "6-9 months" ~ "wasting incidence\nbefore 9 months",
    age_inc == "9-12 months" ~ "wasting incidence\nbefore 12 months"
  )) %>%
  mutate(age_inc_f = factor(age_inc_f, levels = c(
    "wasting incidence\nbefore 3 months",
    "wasting incidence\nbefore 6 months",
    "wasting incidence\nbefore 9 months",
    "wasting incidence\nbefore 12 months"
  )))

plot_data_sub = plot_data_sub %>%
  mutate(age_meas_n = gsub(" month measurement", "", age_meas)) %>%
  mutate(age_meas_n = factor(age_meas_n, levels = c("15", "12", "9", "6", "3")))


inc_histogram_plot = ggplot(plot_data_sub, 
                            aes(x=whz, y = age_meas_n, fill = ..x..)) + 
  geom_density_ridges_gradient(stat = "binline", 
                               binwidth=.1, 
                               scale=0.8,
                               size=0.01) + 
  facet_grid(~age_inc_f) +
  ylab("Measurement age, months")+
  xlab("Length-for-age Z-score")+
  scale_x_continuous(breaks = seq(-5, 3.5, 1), 
                     labels = seq(-5, 3.5, 1)) +
  geom_vline(xintercept = -2, linetype="dashed") +
  scale_fill_viridis(name = "LAZ", option = "magma", direction= -1) 

# to do: add better labels, n, etc

ggsave(inc_histogram_plot, file="figures/wasting/fig_wast_inc_dist_hist.png", width=11, height=6)


# --------------------------------------------
# stacked histogram plot -stratified by birth wasting
# --------------------------------------------
# # drop whz at tails of distributions and subset to kids born wasted
# plot_data_sub = plot_data %>% filter(whz >=-5 & whz <=3.5)
# 
# # rename labels
# plot_data_sub = plot_data_sub %>%
#   mutate(age_inc_f = case_when(
#     age_inc == "0-3 months" ~ "wasting incidence\nat 3 months",
#     age_inc == "3-6 months" ~ "wasting incidence\nat 6 months",
#     age_inc == "6-9 months" ~ "wasting incidence\nat 9 months",
#     age_inc == "9-12 months" ~ "wasting incidence\nat 12 months"
#   )) %>%
#   mutate(age_inc_f = factor(age_inc_f, levels = c(
#     "wasting incidence\nat 3 months",
#     "wasting incidence\nat 6 months",
#     "wasting incidence\nat 9 months",
#     "wasting incidence\nat 12 months"
#   )))
# 
# plot_data_sub = plot_data_sub %>%
#   mutate(age_meas_n = gsub(" month measurement", "", age_meas)) %>%
#   mutate(age_meas_n = factor(age_meas_n, levels = c("15", "12", "9", "6", "3")))


inc_histogram_plot_strat = ggplot(plot_data_sub, 
                            aes(x=whz, y = age_meas_n, fill = ..x..)) + 
  geom_density_ridges_gradient(stat = "binline", 
                               binwidth=.1, 
                               scale=0.8,
                               size=0.01) + 
  facet_grid(born_wasted~age_inc_f) +
  ylab("Measurement age, months")+
  xlab("Length-for-age Z-score")+
  scale_x_continuous(breaks = seq(-5, 3.5, 1), 
                     labels = seq(-5, 3.5, 1)) +
  geom_vline(xintercept = -2, linetype="dashed") +
  scale_fill_viridis(name = "LAZ", option = "magma", direction= -1) 

# to do: add better labels, n, etc

ggsave(inc_histogram_plot_strat, file="figures/wasting/fig_wast_inc_dist_hist_strat.png", width=11, height=6)

# --------------------------------------------
# % wasted / median 
# --------------------------------------------

# function to get % wasted and min, med, max
# for each incidence cohort at each subsequent
# measurement age 
summarize_dist = function(age_incov, data){
  
  data = data %>% filter(age_inc == age_incov) 
  
  tab_age_meas =  table(data$age_meas)
  
  age_meas_list = rev(names(tab_age_meas[tab_age_meas>0]))
  
  res = matrix(NA, nrow = length(age_meas_list), ncol = 8)
  
  for(i in 1:(length(age_meas_list))){
    y = data %>% 
      ungroup() %>%
      filter(age_inc == age_incov & 
               age_meas == age_meas_list[i]) %>%
      mutate(wasted = ifelse(whz < -2 , 1, 0))
    
    prev.cohort = y %>%
      group_by(studyid,country) %>%
      summarise(nmeas=length(unique(subjid)),
                prev=mean(wasted),
                nxprev=sum(wasted==1))
    
    re = fit.rma.inc.cohort(
      data = prev.cohort,
      ni = "nmeas",
      xi = "nxprev",
      measure = "PLO",
      nlab = "children"
    )
    
    res[i,1] = age_incov
    res[i,2] = age_meas_list[i]
    res[i,3] = length(unique(y$studyid))
    res[i,4] = length(unique(y$country))
    res[i,5] = length(unique(y$subjid))
    res[i,6] = re$est
    res[i,7] = re$lb
    res[i,8] = re$ub
    
  }
  
  res = as.data.frame(res)
  colnames(res) = c("age_inc", "age_meas",
                    "nstudy", "ncountry", "nchild",
                    "wasting_prev", "prev_lb", "prev_ub")
  
  res$wasting_prev = as.numeric(as.character(res$wasting_prev))
  res$prev_lb = as.numeric(as.character(res$prev_lb))
  res$prev_ub = as.numeric(as.character(res$prev_ub))
  
  return(res)
  
}

age_inc_list = as.list(levels(plot_data$age_inc))
results_list = lapply(age_inc_list, function(x) summarize_dist(
  data = plot_data, age_inc = x))

results_df = bind_rows(results_list)


saveRDS(results_df, file = paste0(res_dir, "wasting_inc_cohort_summary.RDS"))
