...
source("C:/Users/andre/Documents/HBGDki/ki-longitudinal-manuscripts/0-project-functions/0_risk_factor_shared_functions.R")
# source("/Users/sonali/Desktop/ki-longitudinal-manuscripts/0-project-functions/0_risk_factor_shared_functions.R")
...
# Load data
# try(setwd('/Users/sonali/Desktop/bmgf_shiny_app/'))
# try(wasting_results <- get(load('data/wasting_results.rdata')))
...
try(results <- readRDS('results/rf results/full_RF_results.rds'))



# Transform and subset data for visualization
d <- results
...
#d$agecat <- "unstratified"
d$agecat[is.na(d$agecat)] <- "unstratified"
...
RMAest_FE <- d %>% group_by(intervention_variable, agecat, intervention_level, outcome_variable, unadjusted,birthweight_strat) %>%
...
RMAest_RE_africa <- d %>% filter(Region=="Africa") %>% 
  group_by(intervention_variable, agecat, intervention_level, outcome_variable, unadjusted,birthweight_strat) %>%
  do(try(poolRR(.))) %>% mutate(studyid="Pooled - Africa - RE", Region="Africa", pooled=1) %>% as.data.frame()
RMAest_RE_asia <- d %>% ungroup() %>% filter(Region=="Asia") %>% do(droplevels(.)) %>% 
  group_by(intervention_variable, agecat, intervention_level, outcome_variable, unadjusted,birthweight_strat) %>%
  do(try(poolRR(.))) %>% mutate(studyid="Pooled - Asia - RE", Region="Asia", pooled=1) %>% as.data.frame()
RMAest_RE_latamer <- d %>% filter(Region=="Latin America") %>% 
  group_by(intervention_variable, agecat, intervention_level, outcome_variable, unadjusted,birthweight_strat) %>%
  do(try(poolRR(.))) %>% mutate(studyid="Pooled - Latin America - RE", Region="Latin America", pooled=1) %>% as.data.frame()


RMAest_FE_africa <- d %>% filter(Region=="Africa") %>% 
  group_by(intervention_variable, agecat, intervention_level, outcome_variable, unadjusted,birthweight_strat) %>%
  do(try(poolRR(., method = "FE"))) %>% mutate(studyid="Pooled - Africa - FE", Region="Africa", pooled=1) %>% as.data.frame()
RMAest_FE_asia <- d %>% ungroup() %>% filter(Region=="Asia") %>% do(droplevels(.)) %>% 
  group_by(intervention_variable, agecat, intervention_level, outcome_variable, unadjusted,birthweight_strat) %>%
  do(try(poolRR(., method = "FE"))) %>% mutate(studyid="Pooled - Asia - FE", Region="Asia", pooled=1) %>% as.data.frame()
RMAest_FE_latamer <- d %>% filter(Region=="Latin America") %>% 
  group_by(intervention_variable, agecat, intervention_level, outcome_variable, unadjusted,birthweight_strat) %>%
  do(try(poolRR(., method = "FE"))) %>% mutate(studyid="Pooled - Latin America - FE", Region="Latin America", pooled=1) %>% as.data.frame()
...
d <- bind_rows(d, RMAest_RE_africa, RMAest_RE_asia, RMAest_RE_latamer,  RMAest_FE_africa, RMAest_FE_asia, RMAest_FE_latamer, RMAest_RE, RMAest_FE)
save(d, file="C:/Users/andre/Documents/HBGDki/bmgf_shiny_app/data/uncleaned_pooled_estimates")
...
saveRDS(d, 'C:/Users/andre/Documents/HBGDki/ki-longitudinal-manuscripts/results/rf results/RF_results_processed.rds')
