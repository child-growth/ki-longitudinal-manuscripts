...
calc.ci.agecat <- function(d, range = 3, birth = "yes") {
  ...
  if (range == 3 & birth == "no") {
    d <- d %>%
      ...
      mutate(agecat = factor(agecat, levels = c("1 day-3 months", "3-6 months", "6-9 months", "9-12 months", "12-15 months", "15-18 months", "18-21 months", "21-24 months")))
  }
  if (range == 6 & birth == "no") {
    d <- d %>%
      mutate(agecat = ifelse(agedays==1, "Birth",
       ifelse(agedays > 1 & agedays <= 6 * 30.4167, "0-6 months",
        ifelse(agedays > 6 * 30.4167 & agedays <= 12 * 30.4167, "6-12 months",
          ifelse(agedays > 12 * 30.4167 & agedays <= 18 * 30.4167, "12-18 months",
            ifelse(agedays > 18 * 30.4167 & agedays <= 24 * 30.4167, "18-24 months", "")
          )
