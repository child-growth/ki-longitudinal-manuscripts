...
poolRR <- function(d, family="binomial", method="REML"){
...
#Pooled estimate function for PAR's
poolPAR <- function(d, method="REML"){

      fit<-NULL
      try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method=method, measure="MD"),silent = TRUE)
      if(is.null(fit) & method=="REML"){try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method="ML", measure="MD"),silent = TRUE)}
      if(is.null(fit) & method=="REML"){try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method="HE", measure="MD"),silent = TRUE)}
      if(is.null(fit) & method=="REML"){try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method="EB", measure="MD"),silent = TRUE)}
      
      if(is.null(fit)){
        est <- data.frame(PAR=NA, SE=NA, PAR.CI1=NA, PAR.CI2=NA)
      }else{
        est<-data.frame(fit$b, fit$se)
        colnames(est)<-c("PAR","SE")
        
        est$PAR.CI1<-(est$PAR - 1.96 * est$SE)
        est$PAR.CI2<-(est$PAR + 1.96 * est$SE)
      } 
...
#Pooled estimate function for exposure prevalences
poolRF_prev <- function(d, method="FE"){
  
  fit<-NULL
  try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method=method, measure="PR"),silent = TRUE)
  if(is.null(fit) & method=="REML"){try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method="ML", measure="PR"),silent = TRUE)}
  if(is.null(fit) & method=="REML"){try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method="HE", measure="PR"),silent = TRUE)}
  if(is.null(fit) & method=="REML"){try(fit<-rma(yi=untransformed_estimate, sei=untransformed_se, data=d, method="EB", measure="PR"),silent = TRUE)}
