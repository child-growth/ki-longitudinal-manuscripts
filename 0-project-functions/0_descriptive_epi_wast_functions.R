...
#     summarize(med=median(wasting_duration, na.rm=T), var=var(wasting_duration, na.rm=T), n=n()) %>%
#     mutate(se=sqrt(var), ci.lb=med - 1.96 * se, ci.ub=med + 1.96 * se,
#            nmeas.f=paste0("N=",n," children"))
#   
#   pooled.vel=lapply(agelist,function(x) 
#     fit.cont.rma(data=df,yi="med", vi="var", ni="n",age=x, nlab="children"))
...
#     subset(., select = -c(se)) %>%
#     rename(strata=agecat, Median=est, N=nmeas, Lower.95.CI=lb, Upper.95.CI=ub) %>% as.data.frame()
#   print(pooled.vel)
#   
#   cohort.df <- df %>% subset(., select = c(studyid, country, region, agecat, n, nmeas.f, mean, ci.lb, ci.ub)) %>%
#     rename(N=n, Median=med, Lower.95.CI=ci.lb, Upper.95.CI=ci.ub,
#            strata=agecat) %>%
